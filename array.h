...
void Array<ElementType>::push_back(const ElementType& X) {
	if (Len + 1 > maxLen) {
		// Íå õâàòàåò ïàìÿòè, íåîáõîäèìî âûäåëèòü íîâóþ è ñêîïèðîâàòü òóäà ñòàðóþ èíôîðìàöèþ
...
void Array<ElementType>::erase(typename const Array<ElementType>::iterator& pos) {
	if (pos == end()) return;
	ElementType* cur = pos.current; // Ýòî âîçìîæíî èç-çà òîãî, ÷òî íàïèñàëè friend class Array;
...
//-------------------------------------------------------------------------------------
template <typename ElementType>
Array<ElementType>::Array(const Array<ElementType>& X) {
	Len = X.Len;
	maxLen = X.maxLen;
	isSorted = X.isSorted;
	data = new ElementType[maxLen];
...
//-------------------------------------------------------------------------------------
template <typename ElementType>
Array<ElementType>::Array(Array<ElementType>&& X) {
	Len = X.Len;
	maxLen = X.maxLen;
	isSorted = X.isSorted;
	data = X.data;
...
template <typename ElementType>
Array<ElementType>& Array<ElementType>::operator=(const Array<ElementType>& X) {
	if (this != &X) {
		if (data) delete[] data;
...
//-------------------------------------------------------------------------------------
template <typename ElementType>
Array<ElementType>& Array<ElementType>::operator=(Array<ElementType>&& X) {
	if (this != &X) {
		if (data) delete[] data;
